...
# GOOGLE SHEETS - PLANILHA DE PLACAS E DESTINO DOS RESULTADOS
# A mesma planilha é usada para ler as placas e gravar os desvios.
GOOGLE_SHEETS_SPREADSHEET_ID = os.getenv(
    "GOOGLE_SHEETS_SPREADSHEET_ID",
    "1q-SxCJ4C97uEzuPUyvykM8maMw0VxF540nwfK7bv0SA",
).strip()

GOOGLE_SHEETS_ABA_PLACAS = os.getenv(
    "GOOGLE_SHEETS_ABA_PLACAS",
    "Geral",
).strip()

GOOGLE_SHEETS_COLUNA_PLACA = os.getenv(
    "GOOGLE_SHEETS_COLUNA_PLACA",
    "PLACA",
).strip()
...
# As placas são carregadas automaticamente da aba configurada acima.
PLACAS = []
...
# ============================================================
# CARREGAR PLACAS DA MESMA GOOGLE SHEETS
# ============================================================

def carregar_placas_google():
    """Lê as placas da aba configurada na mesma planilha dos resultados."""
    global PLACAS

    print("\n" + "=" * 70)
    print("CARREGANDO PLACAS DA GOOGLE SHEETS")
    print("=" * 70)
    print(f"Planilha ID: {GOOGLE_SHEETS_SPREADSHEET_ID}")
    print(f"Aba: {GOOGLE_SHEETS_ABA_PLACAS}")
    print(f"Coluna: {GOOGLE_SHEETS_COLUNA_PLACA}")
    ...
    try:
        aba = planilha.worksheet(GOOGLE_SHEETS_ABA_PLACAS)
    except gspread.WorksheetNotFound as erro:
        raise RuntimeError(
            f'Aba "{GOOGLE_SHEETS_ABA_PLACAS}" não encontrada na planilha.'
        ) from erro

    valores = aba.get_all_values()
    if not valores:
        raise RuntimeError(f'A aba "{GOOGLE_SHEETS_ABA_PLACAS}" está vazia.')

    cabecalhos = [str(v).strip() for v in valores[0]]
    alvo = normalizar_texto(GOOGLE_SHEETS_COLUNA_PLACA)
    indice_placa = None

    for i, cabecalho in enumerate(cabecalhos):
        if normalizar_texto(cabecalho) == alvo:
            indice_placa = i
            break

    if indice_placa is None:
        for i, cabecalho in enumerate(cabecalhos):
            if "PLACA" in normalizar_texto(cabecalho):
                indice_placa = i
                break

    if indice_placa is None:
        raise RuntimeError(
            f'Coluna "{GOOGLE_SHEETS_COLUNA_PLACA}" não encontrada. '
            f"Colunas encontradas: {cabecalhos}"
        )

    placas = []
    for linha in valores[1:]:
        if indice_placa >= len(linha):
            continue
        placa = normalizar_placa(linha[indice_placa])
        if placa and placa not in placas:
            placas.append(placa)

    if not placas:
        raise RuntimeError(
            f'Nenhuma placa válida foi encontrada na aba "{GOOGLE_SHEETS_ABA_PLACAS}".'
        )

    PLACAS = placas
    print(f"✓ {len(PLACAS)} placa(s) carregada(s) da planilha.")
...
def executar_auditoria(arquivos_html, inicio, fim):
    ...
    # Envia os resultados para a MESMA Google Sheets de onde
    # as placas foram carregadas. A aba Geral não é alterada.
...
def main():
    ...
    print(f"Planilha de placas/resultados: {GOOGLE_SHEETS_SPREADSHEET_ID}")
    print(f"Aba de placas: {GOOGLE_SHEETS_ABA_PLACAS}")
    print(f"Coluna de placas: {GOOGLE_SHEETS_COLUNA_PLACA}")

    # Lê as placas diretamente da mesma planilha onde os resultados serão gravados.
    carregar_placas_google()
    validar_placas()
